...
BOOL CServer::InitAndListen(HWND hwnd,UINT port)
{
	...
	//	m_hSocket = socket(AF_INET, SOCK_STREAM,0);//SOCK_STREAM是面向连接的啊
	//	m_hSocket = socket(AF_INET, SOCK_DGRAM,0);//不是listen ERROR就是binding error
	...
	//开始一个侦听过程，等待客户的连接
	ret = listen(m_hSocket, 5);    //这里的第二个参数表示最多支持的客户同时申请连接数
	if(ret == SOCKET_ERROR)
	{	//listen失败
